---
name: literary-maverick
description: Write and revise prose in a bold, vivid "literary maverick" voice — fresh imagery, strong openings and closings, living verbs instead of abstract nouns, and the confidence to break folklore grammar rules while keeping the writing clear. Centers on an iterative rewrite loop that takes a phrase, sentence, or passage and refines it across multiple passes, testing each candidate for relevance, effectiveness, and appropriateness. Use this whenever the user wants writing that is distinctive, punchy, or literary; wants a draft "punched up," "sharpened," or "made less flat / corporate / generic"; wants phrasing refined or iterated on; wants help with an opening line, headline, tagline, or closing; or is drafting essays, articles, blog posts, or any prose where style matters and a plain first draft is not enough. Reach for it even when the user never says the words "literary" or "maverick" but clearly wants prose with more life in it.
---
...
1. **Relevance** — Does it serve *the point*? Does it connect to the sentences around it, advance the argument, earn its place? A dazzling line that pulls toward a tangent fails this test no matter how good it sounds.
2. **Effectiveness** — Does it *land*? Is it clear, concrete, rhythmic, memorable? Does it show rather than announce, end strong, and carry some spark? This is where the maverick boldness lives — vividness, surprise, a fresh image.
3. **Appropriateness** — Does it *fit*? Right register for the audience and genre, accurate (not vivid-but-false), and not gratuitously offensive or misleading. This is the test that reins boldness back in. Edgy-for-its-own-sake fails here.

### The procedure

1. **Diagnose.** Read the current version against the three tests and the craft qualities above. Name what's actually weak — "abstract and hedged," "buries the point," "tonally wrong for a toast," "flat ending." Don't rewrite blind.
2. **Propose.** Write one concrete rewrite that targets the weakness you named. Change something real; don't just shuffle words.
3. **Judge.** Score the new version on relevance, effectiveness, appropriateness. Did it improve at least one axis *without losing ground* on the others?
4. **Decide.**
...
5. **Deliver** the winning version. If the user is iterating with you, a one-line note on *why* it's the strongest pass is often useful; if they just want the line, give them the line.
...
**Example 1 — the routine punch-up.**
Context: opening line of a blog post for a general audience about remote work.
Input: *"In today's fast-paced business environment, many organizations are increasingly leveraging remote work arrangements to optimize productivity outcomes."*
...
**Example 2 — appropriateness reins in the boldness.**
Context: the opening of a wedding toast.
Input: *"Marriage is the tomb of love, or so the cynics croak from their lonely crypts."*
...
**Example 3 — knowing when *not* to rewrite.**
Context: a line in a personal essay.
Input: *"Grief is a country with no exit visas."*
...
**Contents**
1. Folklore rules you can break (and why they're folklore)
...
## Zooming out: the whole piece, and the whole course

The rewrite loop above works on a phrase, a sentence, a passage. But a piece can pass every local test and still fail as a whole — and a *course* of many pieces fails in ways no single piece can reveal. Two larger scopes need their own discipline.

### The whole piece

Before calling a single page (a module, an essay, a post) done, read it start to finish as one object and check what only the whole reveals:

- **Logical coherence.** No section may contradict another. If you define a thing one way and later describe it as if it could be otherwise, a sharp reader trips. Trace the claims across sections, not just within them.
- **One idea, one image.** A vivid metaphor used once lands; the same idea dressed in five different metaphors across one page is busyness, not richness. Pick the strongest image for each idea and let the others go.
- **No word or image worn flat.** Count your tics. If "machine" appears eleven times, or one phrase opens three paragraphs, the repetition has stopped being emphasis and become wallpaper. (Domain terms that *must* repeat are exempt — "group" in a group-theory lesson is not a tic.)
- **A strong opening *and* a strong close.** The loop guards opening lines; it must guard closings too. End the whole piece on a strong, stressed word — not an administrative mumble ("…in the next section") and not a deflating "is just a."
- **Don't expose the scaffolding.** Section labels that name the *device* ("The hook", "Intuition") belong in the outline, not as visible headings; classic style shows the thing, it doesn't announce the move.

#### Metaphors must point true, pay off, and bridge

An image is a loan against the reader's trust, and a bad one costs more than it buys. Before you let a metaphor carry an idea, hold it to three tests — and if it can't clear all three, cut it. A clean concrete *shape* the definition can be read straight off beats a vivid image that misleads.

- **Point true.** The picture must aim at exactly **one** correct real-world referent and drag no wrong association in behind it. "A machine with slots and a crank" sounds harmless until you hear the slot machine in it — gambling, chance, randomness — the exact opposite of the deterministic operation you meant to show. The reader's first instinct is the one that sticks; if the nearest association is the wrong one, the image has already lied. One referent, and the right one, or none.
- **Pay it off.** A metaphor announced as central and then dropped is a debt left unpaid — the reader keeps waiting for the crank to turn again and it never does. If you raise an image, develop it and reuse it; don't strand it after one sentence.
- **Cash it in.** Every piece of the image must map onto the thing it motivates. If the machine has two slots, say what the two slots *are*; if it has a crank, say what turning the crank *does*. An image that isn't spent on the formal content is decoration, and decoration in a lesson is noise.

When no image survives all three, write none. A plain, exact shape — "two things go in, one comes out, and what comes out is fixed by what went in" — is not a failure of nerve; it's the honest version of what the broken metaphor was pretending to do. And wherever an intuition hands off to a formal definition, owe the reader the connective passage that maps the picture onto the notation, piece by piece — never set an image down beside a definition and trust them to find each other. (That handoff is the *bridge*; `_config/math-style.md` makes it a rule.)

### The whole course

When many pieces share one reader and one subject, they must read as one book, not as a stack of strangers. Before a course ships, hold the whole set to these:

- **One voice.** The narrator's temperature must not wander from warm and storytelling to dry and clinical between pieces. Level it.
- **Name everything the same way, everywhere.** One symbol per object, one name per recurring thing, across every piece — a thing renamed between consecutive pieces (the same object called `s` here and `f` there) reads as a *new* thing. This is the cross-piece form of "name each recurring thing the same way."
- **Remind, don't re-derive.** Reminders are good — readers forget, and a one-line callback ("the result we built earlier") earns its place. Re-teaching a settled result from scratch in piece after piece is the opposite: it tells the reader you don't trust them to have learned. Decide once which piece *owns* each idea; everywhere else, call back.
- **Signpost only when it pays.** A pointer forward or back is worth it when it genuinely aids learning; the same "here's what's coming" promise made in piece after piece is noise. Cap it.
- **Vary the edges.** If every piece opens with the same kind of move and closes with the same formula, the set feels machine-stamped. Keep each opening and closing strong *and* different.

These whole-piece and whole-course checks are not the writer's job alone — when work is produced in parallel, no single writer can see them. They belong to a dedicated editorial pass that reads everything together, against a shared canon that fixes the names, the owners, the metaphors, and the caps before anyone writes.
