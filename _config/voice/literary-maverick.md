...
**No em dashes, and recompose rather than chop.** Do not use the em dash (`—`) in prose. But removing one is a *rewrite*, not a find-and-replace. The dash is a clause-stuffer, and the failure mode when you delete it is to leave the stuffing behind in a worse container: a full stop that strands a fragment (*Built, not grown.* / *Market: spontaneous. State: intrusion.*), or a colon or semicolon doing the dash's old job so often that the page fills with them. That is not an improvement; it just trades one tic for another. Instead **recast the whole sentence** so the thought flows without the break: fold the aside in with commas, promote it to its own real sentence with a subject and a verb, or cut it. Use a colon only to genuinely introduce what follows, a semicolon sparingly, parentheses only for a true aside. If a sentence seems to *need* a dash, it is usually two sentences badly joined or one sentence overloaded. Fix the underlying sentence; do not just swap the punctuation. (Hard constraint, enforced by `lint-prose.mjs` check 3. The one exception is a dash inside a quotation, reproduced faithfully.)

**Don't narrate the structure, don't direct the reader, and don't recite the method.** Classic style shows the reader the thing; it does not stand beside the thing announcing it, pointing at it, or grading its own honesty. Three habits to cut:
- **Self-narration.** Sentences that describe the text instead of being it: *This module is about…*, *Here is the question this section exists to answer*, *A warning before the two theories*, *Now do the thing this whole module exists for*. Just do it.
- **Directive metadiscourse.** Standing beside the reader issuing instructions: *Notice that…*, *Hold that in mind*, *Hold onto this*, *Keep X in view*, *Remember that…*, *Carry this with you*, *Mark the seam*. These tell the reader to do the noticing that good prose makes happen on its own. Write the sentence so the point lands, and the reader notices without being told to. (A genuine retrieval prompt in a `<details>` check is different and stays; this is about the running prose.)
- **Reciting the discipline.** Naming your own analytic moves as you make them, or listing them as takeaways: *Now mark the seam between evidence and interpretation*, *Two disciplines came with that. First… Second…*. **Enact a discipline; never announce it.** Keep evidence and interpretation genuinely distinct in the argument instead of repeatedly saying you are doing so. Name a seam at most once in a whole piece, and never as a recurring phrase.

And three devices go stale fast when a piece, or a whole course, leans on them: the **rhetorical question** (*How does a made thing come to feel natural?*), **direct second-person address** (*you stood, you worked out, you pocketed*), and the **dramatic one-clause fragment** (*Built, not grown.*). Each earns its place once or twice for emphasis. Used as a default move, paragraph after paragraph, they become a mannerism the reader hears louder than the content. Keep a tight budget for all of these, and vary how each passage opens and turns rather than reaching for the same gesture every time.
...
A page can pass every local test and still fail as a whole, and a *course* of many pieces fails in ways no single writer can see. That larger discipline — whole-piece coherence, the metaphor point-true / pay-off / bridge tests, and the whole-course rules (one voice, name-everything-the-same, remind-don't-re-derive, density, confidence-not-comfort, show-the-artifact, vary the edges) — is a different scope with a different owner: **the editorial stage (`pipeline/04_editorial`).** It now lives in its own reference, mastered there: **`_config/voice/editorial-craft.md`**.

This file is the engine for the phrase, the sentence, and the passage. That file is the engine for the book. (A module author still vets a single module's own metaphor and bridge — the per-module image rule is in `_config/course-design.md`; the full three tests live in editorial-craft.)
