...
**Ban the em dash.** Do not use the em dash (`—`) in prose. It is a clause-stuffer: it lets you bolt a second or third idea onto a sentence that was already complete, and it asks the reader to hold every clause at once, which is exactly the working-memory load the rule above tells you to cut. The fix is almost always a full stop: end the sentence and start the next one. Where a full stop is too heavy, a comma, a colon (to introduce what follows), or parentheses (for a true aside) will carry the join. This is a hard constraint the prose lint enforces (`lint-prose.mjs`, check 3), not a preference; the only exception is a dash inside a quotation, where the source's own punctuation is reproduced faithfully. If you reach for a dash, you are usually trying to fit two sentences into one. Write the two sentences.
