...
Now the part you must not skim — read it twice. The invariant gives you a one-way test, and the direction is everything.
...
Take $U = (0,1)$ and $V = (1,2)$. Both are open intervals, hence open. They are disjoint (the point $1$ belongs to neither — it has been removed from $Z$). Their union is exactly $Z$. Neither is empty. So $U, V$ split $Z$, and $Z$ is disconnected. (The single missing point at $1$ breaks the interval into two separated pieces — there is no clinging boundary point holding them together, because $1$ is not in the space at all.)
