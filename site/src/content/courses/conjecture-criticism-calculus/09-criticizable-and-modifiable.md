...
A **probability distribution** over a set $X$ is an assignment of a number $\Pr[x] \in [0,1]$ to each element $x \in X$, with the numbers summing to 1. Think of it as a random-choice device: when you "sample from the distribution," you get one element of $X$, with each element appearing as often as its assigned probability.
...
The one condition that matters here: $\Pr[E] > 0$ means the event is *possible* — the sampler can produce some element in $E$. It does not mean $E$ is likely or common; it means $E$ is not ruled out with certainty. Contrast $\Pr[E] = 0$: no element in $E$ is ever produced. For a finite distribution, $\Pr[E] = 0$ means no element of $E$ has any weight, so it simply never outputs anything in $E$.
...
**Bridge.** The variation kernel $\mu(\cdot \mid a)$ is the random-edit machine for $a$. The event $\{a' \not\approx_B a\}$ is the set of all sampled edits that produce a battery-inequivalent variant — something the battery can actually distinguish from $a$. The condition $> 0$ says this event is *possible*: the kernel can produce a genuine alternative, not just a rewording. If the probability were 0, every edit of $a$ would be battery-equivalent to it — the artifact's content space, as far as the battery is concerned, is a single point.
