...
## REDO DIRECTIVE (2026-06-18): level dial + show-the-artifact + precedence

This course is **`highschool`, no proof experience** — the beginner end of the level dial
(`_config/course-design.md` §2). Binding additions for this run, on top of the symbol registry above:

- **Define every term on first use** (group, set, function, binary operation, closure, associativity,
  identity, inverse, modular arithmetic, subgroup, order, coset, cyclic, generator, homomorphism,
  isomorphism, ring, field). No orphan terms. Introduce *proof* gently (module 09).
- **Every module ships an artifact AND an analogy/picture.** Cayley/operation tables, worked numeric
  computations, the symmetry picture. Module 06 reuses `./assets/triangle-symmetries.svg` (alt text required).
- **Callbacks re-gloss** the term in a few words; a beginner has forgotten.
- **De-soothe** (lint 5–6 clean): no soothing manner adverbs, no hand-holding.
- **Precedence:** prose leads stakes / analogies / historical-reach context / transitions; pedagogy leads
  definitions, tables, the Lagrange argument.

### Per-module artifact + picture (minimum)
| Module | Artifact / picture (minimum) |
|--------|------------------------------|
| 01 secret life of symmetry | a seen symmetry (square/coaster turn, clock, triangle) — the three disguises |
| 02 clock arithmetic | a clock face + a `$\mathbb{Z}_n$` addition table; worked mod sums |
| 03 operations & rules | operation tables; a worked closure/identity/inverse check; a non-example |
| 04 what is a group | the 4 axioms checked on `$\mathbb{Z}_4$`; one non-group non-example |
| 05 a zoo of groups | a comparison table of example groups; a small Cayley table |
| 06 symmetries of a triangle | **the triangle-symmetries.svg** + the `$D_3$` Cayley table; "flip-then-spin ≠ spin-then-flip" worked |
| 07 subgroups | the subgroup test worked on examples; a subgroup-inside-group picture |
| 08 cyclic groups & generators | a table of powers of a generator; `$\mathbb{Z}_n$` traced as cyclic |
| 09 Lagrange's theorem | a coset-partition picture + worked cosets; the argument, every step in prose |
| 10 homomorphisms & isomorphisms | a worked homomorphism; an isomorphism shown by matching two Cayley tables |
| 11 rings & fields | `$\mathbb{Z}$` as a ring; a small finite field; the two-operation table |
