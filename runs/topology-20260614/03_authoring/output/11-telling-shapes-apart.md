...
The **sphere** and the **doughnut** are not the same shape. We do not need to wrestle with deforming one into the other; we just read the Euler characteristic. By module 10, $\chi(\text{sphere}) = 2$ and $\chi(\text{torus}) = 0$, and the doughnut's surface *is* the torus. Since
