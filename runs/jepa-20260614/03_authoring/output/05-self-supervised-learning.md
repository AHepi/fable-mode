...
Last module we watched a machine learn. It starts useless, it makes a guess, we measure how
...
writes down "dog," and hands that to the machine to be graded against. And for one whole
branch of machine learning, that is exactly the deal. The trouble is, it doesn't scale to the
thing the first module promised — a machine that learns about the world *on its own*, from the
...
Let's lay the two arrangements side by side, because the contrast is the whole point and it's
easy to blur.
...
already knew. The machine guesses "cat," gets measured against the human's "cat," and is nudged.
...
hide part of it from the machine — blot out a patch of the photo, cover the next word of the
sentence. Then you ask the machine to predict the hidden part from the part still showing. And
...
> **A tempting wrong answer:** "Self-supervised" sounds like the machine is off on its own with
...
We hand the machine everything except that last word and ask it to predict what's missing. It
...
Multiply that by billions of sentences, and the machine is being drilled on an endless,
self-grading quiz that no person ever wrote. The same recipe works on a photo: hide a square
patch and ask the machine to predict what belonged there, using the rest of the image as the
...
A photo of a beach, with no caption or label of any kind, is used to train a machine
...
just the machine poking around with no goal." Fix the second half of that sentence.
...
(a) a person tags ten thousand photos as "cat" or "dog," and the machine learns to tell them
