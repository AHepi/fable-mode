...
Six modules in, we've been circling one idea without ever quite landing on it. We have all
the parts now — they're sitting on the table in front of us — and this is the module where we
finally bolt them together into the machine the whole course is named after. So let me say at
the outset what we're about to build, in one breath, and then spend the rest of the module
making every word of it earned: **JEPA fills a blank by predicting the gist of what's missing,
and it does the predicting in representation space, never in pixels.** That's it. That's the
whole architecture. Everything below is just unpacking that sentence slowly enough that you
could draw it on a napkin afterward.
...
free question: a **visible part** (what we let the machine see) and a **hidden part** (what we
...
machine reasons from. (It's the same kind of box from module 02: input in, numbers out.)
...
alone, which is why filling the blank well is evidence the machine learned something real.
...
3. **Predictor.** The predictor takes the context representation and produces its guess — a list
   ...
   in it should say, in the only language the machine speaks, *leafy, lit from the upper left,
...
pixels would put us right back on the generative road, grading the machine on whether it nailed
every unpredictable speck (module 06's waste). Turning the hidden part into a representation first
throws that unpredictable surface detail away and keeps the gist, so the machine is graded only on
...
toward another. Picturing it as the brain overstates its role and misreads the machine: the
...
keeping the machine from cheating, which is module 09.)
...
turns a prediction back into a picture. The natural next question is the one we've been postponing
on purpose: granting that JEPA *can* work this way — why is predicting the gist actually *better*
than rebuilding the pixels? That payoff is where we go next.
