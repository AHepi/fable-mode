...
what addition gives you on the integers: closure, a do-nothing zero, an opposite for every
number that cancels it. We proved as much back when we first defined a group.
...
are a ring but not a field. Hold those two facts; the whole module is unpacking them.
...
Here is the question the whole module has been circling. Is $\mathbb{Z}_6$ a field?
...
field can never allow. The contrast with $\mathbb{Z}_5$ is the whole point: **$\mathbb{Z}_p$
...
$2x = 1$ has no solution either, for exactly the reason you found above. The whole comfort
of "just divide both sides" is a privilege fields grant and rings withhold. Naming the
structure tells you, in advance, which equations you are allowed to solve.
...
In $\mathbb{Z}_6$, the element $2$ has no multiplicative inverse (no $x$ gives
$2x \equiv 1$), so the field condition fails — one nonzero element without an inverse is
enough. The sharpest symptom is $2 \cdot 3 = 0$: two nonzero elements whose product is
zero, which a field forbids outright. It happens because $6 = 2 \times 3$ is *composite*. By
contrast $5$ is prime, so $\mathbb{Z}_5$ has no such factor pairs, every nonzero element is
invertible, and $\mathbb{Z}_5$ is a field. The rule to carry away: **$\mathbb{Z}_n$ is a
