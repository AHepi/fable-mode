...
An inverse is the *undo* element: combine $a$ with its inverse and you land back on the do-nothing element. Under ordinary addition the inverse of $a$ is $-a$, since $a + (-a) = 0$. On the $12$-hour clock the inverse of $5$ is $7$: $5 + 7 = 12 = 0$. Wind forward five hours, then seven more, and you have gone all the way around to where you started. Identity and inverse are easy to confuse, so keep them straight: the identity does *nothing*; an inverse *cancels a specific element* back to that nothing.
