...
That is the whole idea of this module. A group can shelter smaller groups inside itself, complete and self-sufficient, like a room inside a house. Find those rooms and you start to see the floor plan — the hidden structure that makes one group different from another. We have a name for such a room: a **subgroup**.
...
For the handful to stand on its own as a group, three things have to hold, and they are exactly the things that make any group a group:

- The identity has to be present — every group needs its "do nothing" element.
...
Let $G$ be a group with operation $\ast$, and let $H$ be a subset of $G$, written $H \subseteq G$. Then $H$ is a **subgroup** of $G$ if $H$ is itself a group under the same operation $\ast$. We write $H \leq G$.

Concretely, $H \leq G$ exactly when all three hold:

1. **Identity.** The identity element $e$ of $G$ lies in $H$.
2. **Closure.** For all $a, b \in H$, the combination $a \ast b$ lies in $H$.
...
All three conditions pass, so $H = \{0,2,4\} \leq \mathbb{Z}_6$. Notice its size: $|H| = 3$, while $|\mathbb{Z}_6| = 6$. Hold onto that pair of numbers — $3$ and $6$ — we'll come back to it.
...
Subgroups aren't a numbers-only phenomenon. Recall $D_3$, the six symmetries of an equilateral triangle: the identity $e$, two rotations $r$ (by $120^\circ$) and $r^2$ (by $240^\circ$), and three reflections (flips). Composition of symmetries is the operation.

Look at just the rotations: $R = \{e, r, r^2\}$. Do them in succession and you only ever get another rotation — spinning then spinning again is still a spin. Concretely, $r \ast r = r^2$, and $r^2 \ast r = r^3 = e$ (three turns of $120^\circ$ bring the triangle home). The identity $e$ is present; composing any two rotations gives a rotation in $R$; and each rotation's inverse is a rotation ($r$ and $r^2$ undo each other, $e$ undoes itself). All three conditions hold, so
...
In $D_3$, is $\{e, f\}$ a subgroup, where $f$ is a single reflection (a flip)?

<details><summary>Show answer</summary>

Yes. A flip done twice returns the triangle to its starting position, so $f \ast f = e$. The identity $e$ is present; composing the two elements stays inside ($e \ast f = f$, $f \ast f = e$, both in the set); and $f$ is its own inverse while $e$ is its own inverse. All three conditions hold, so $\{e, f\} \leq D_3$, with size $|\{e,f\}| = 2$.
...
Containing the identity is necessary but not sufficient — you must also check closure. Here $2 + 5 = 7 \equiv 1$, and $1 \notin \{0,2,4,5\}$. The sum escaped the set, so closure fails and the subset is not a subgroup. (You could stop at the first escape; one failure is enough.) This is the same trap as $\{0,2,3\}$ earlier: the identity is present, but the set leaks.
