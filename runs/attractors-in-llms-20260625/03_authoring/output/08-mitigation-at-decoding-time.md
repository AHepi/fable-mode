...
This module is concrete on purpose. We take that one table and watch each knob bend it.
...
**Contrastive search**, from the SimCTG work, scores a candidate as its model confidence *minus* a degeneration penalty. The penalty is the maximum similarity between the candidate's representation and the representations of the tokens already generated. A token that the model likes but that looks, in representation space, almost identical to something it just said gets marked down. This pushes generation toward tokens that are both probable and *distinct* from the recent past, which is anti-repetition built into the selection rule rather than bolted on as a penalty. It works best on models trained with the matching SimCTG objective, which spreads the token representations out so that "similar representation" actually means "redundant." It also costs extra computation at every step, since you must compare each candidate against the running history.

**Contrastive decoding**, from Li and colleagues, contrasts two *models* instead of one model against its own history. Run a strong "expert" model and a weak "amateur" model on the same context, and generate from the *difference* in their log-probabilities. The degenerate, generic, repetitive habits tend to be shared by both, so subtracting the amateur cancels them, while the expert's genuine competence survives the subtraction. It sharpens open-ended generation, but you now pay to run a second model, and a plausibility constraint has to keep the contrast from chasing tokens the expert finds absurd.
...
The table is also a decision procedure. Name the failure first, then read across. Your output is looping the same phrase: that is self-conditioning, so a repetition penalty or no-repeat-n-gram hits it where it lives, and contrastive search hits it more cleanly if you have the model for it. Your output is bland and templated across many prompts, the mode collapse of module 4: that is a mass-concentration problem, so raise temperature or open the nucleus with a higher $p$. Your output drifts into nonsense because diversity ran too hot: tighten $p$ or lower $\tau$. No single knob is the answer to every basin, because the basins are not all the same shape.
