...
## 7. Prose ↔ pedagogy precedence (this redo)

Per `_config/course-design.md` ("When prose and pedagogy conflict"): **prose leads** on **stakes,
analogies, historical/philosophical context, and inter-module transitions**; **pedagogy leads
everywhere else.** For this course:

- **Stakes** live in M1 and the course intro — the bet that representation-prediction, not pixel
  reconstruction, is the right path to machine intelligence. Craft it; do not reduce it to a mechanism.
- **Historical / philosophical context** is owned by M6 (LeCun's world-model thesis), with framing
  touches allowed elsewhere; give it room — density does not cut it.
- **Analogies**, where used, are crafted (point true, pay off) and hand off to the artifact; still no
  carousel (most modules open another way).
- **Transitions** between modules are prose-led (crafted recaps and bridges).
- **Everywhere else** — the architecture, the loss, collapse/EMA, worked numbers, pseudocode,
  definitions, checks, exercises, density, show-the-artifact — pedagogy leads, and a prose flourish
  that blurs the teaching yields. The soothing register and manner-adverb tics stay banned in every zone.
