...
<details><summary>Check yourself: if the target encoder is updated slowly (EMA, as in M3), does the representation it produces stay useful over training?</summary>

Yes. The EMA update keeps $f_t$ close to $f_c$ — it trails the context encoder but does not diverge from it. As $f_c$ learns better representations, $f_t$ gradually follows. The representations $f_t$ produces remain valid targets for $f_c$ to predict throughout training. If $f_t$ were held completely fixed from the start, it would eventually become a poor target as $f_c$ improved past it; the slow update keeps the targets sensible.
