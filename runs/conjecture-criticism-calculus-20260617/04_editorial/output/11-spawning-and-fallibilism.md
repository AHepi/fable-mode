...
Every prior module assembled the machinery that can now fire: the grounded extension $G$ we built in module 7, the adjudication rule (Adj) that keeps statuses current, the hard-to-vary measure $\mathrm{HV}_B(a)$ and the reach $R_t(a)$ from module 10. The (Spawn) rule watches for conditions on the current state and, when it sees one, registers a new problem $\pi$ with a provenance record tracing it to its cause.

Here is the rule precisely.

## Rule (Spawn)

**On any trigger, register a new problem:** for a trigger condition $C$ detected in state $S = (A, \Pi, \mathrm{addr}, \mathrm{att}, W)$, (Spawn) produces the transition

$$
S \;\rightsquigarrow\; S' = (A,\; \Pi \cup \{\pi_{\text{new}}\},\; \mathrm{addr},\; \mathrm{att},\; W)
$$

where $\pi_{\text{new}} = (d_{\pi_{\text{new}}},\; \mathrm{Crit}_{\pi_{\text{new}}},\; \mathrm{prov}_{\pi_{\text{new}}})$ and $\mathrm{prov}_{\pi_{\text{new}}}$ records which trigger caused the registration.

Only $\Pi$ grows: the artifacts $A$, the addressing relation $\mathrm{addr}$, the attack relation $\mathrm{att}$, and the warrant map $W$ are all carried over unchanged. The state gains one new question and nothing else.
...
$$
S \;\rightsquigarrow\; S' = (A \cup \{\pi_{\text{rule}},\; \pi_{\text{dem}},\; \pi_{\text{adj}},\;\ldots\},\; \Pi,\; \mathrm{addr}',\; \mathrm{att}',\; W')
$$

where each rule is an artifact $r = (c(r), I(r))$ subject to (Crit), and the addresses, attacks, and warrants extend accordingly.

The rule-artifacts join $A$; the problem set $\Pi$ is unchanged by this step. Everything that was already in the state carries forward — (Refl) adds, never deletes.
...
This is not a paradox. The grounded extension $G$ is computed on whatever attack graph $\mathrm{att}$ the state contains at each moment. (Refl) adds the rule-artifacts to $A$; (Crit) can add attacks on them; (Adj) recomputes $G$ to include or exclude them. The system remains well-formed throughout. It just gains the property that no rule sits above the process — every rule is inside it.
...
**After (Adj):** Using the grounded extension $G$ we built in module 7, the chain $j \to k \to a$ gives $G = \{j, a\}$: $j$ is unattacked and enters first; $j$ defeats $k$, so $k$ is refuted; with $k$ refuted, $a$ is reinstated. This is the same configuration the Reinstatement Lemma (module 8) demonstrated.
...
"Proposable" means (Conj) can introduce $a'$; "strictly extended battery" means $a'$ answers to more commitments than $a$ does. In plain terms: there is always a next question to ask. No matter how well $a$ has done — how many problems it addresses, how high its $\mathrm{HV}_B(a)$, how wide its reach $R_t(a)$ — there is always a variation of it that has committed to more and can be proposed and criticized.
...
is a position in an open process, not a fixed point of it. At no $t$ does the trajectory terminate. The grounded extension $G_t$ shifts as attacks arrive; the $\mathrm{HV}$-profile shifts as batteries grow; the reach $R_t$ shifts as the problem library expands. The system's output is a trajectory, not an endpoint.
...
But now $k \in G$ and $k$ attacks $a$, so $a$ is **refuted** by the status definition (there exists $b \in G$ with $(b,a) \in \mathrm{att}$, namely $b = k$). A previous configuration had $a$ accepted; now $a$ is refuted. The status exited accepted $\to$ refuted because the attack graph changed — $j$ (which was defending $a$ by defeating $k$) is now itself attacked by $m$ and thereby loses the ability to defend. This is N1 in action: accepted is not absorbing.
...
**Initial state.** Artifacts: $a$ (conjecture), $k$ (criticism of $a$). Attack: $k \to a$. $k$ is unattacked. Run (Adj): $F(\emptyset) = \{k\}$ (unattacked). $F(\{k\})$: $a$ is attacked by $k \in \{k\}$, so $a \notin F(\{k\})$. Fixed point: $G = \{k\}$. Status: $k$ accepted, $a$ **refuted**.

**Step 2: add $j$ attacking $k$.** Register a new artifact $j$ with a warranted attack on $k$: $(j, k) \in \mathrm{att}$. $j$ is unattacked. Run (Adj): $F(\emptyset) = \{j\}$. $F(\{j\})$: $k$ is attacked by $j \in \{j\}$, so $k \in F(\{j\})$. $a$ is attacked by $k \notin \{j\}$, so $a \notin F(\{j\})$. $F(\{j\}) = \{j, k\}$. $F(\{j,k\})$: every attacker of $a$ is $k$; $k \in \{j,k\}$ and $k$ is itself attacked by $j \in \{j,k\}$, so $a$'s attacker is defeated — $a \in F(\{j,k\})$. Fixed point: $G = \{j, a\}$ (with $k$ refuted). Status: $j$ accepted, $a$ **accepted**.

**Step 3: add $m$ attacking $j$.** Register $m$ with a warranted attack on $j$: $(m, j) \in \mathrm{att}$. $m$ is unattacked. Run (Adj): $F(\emptyset) = \{m\}$. $F(\{m\}) = \{m, j\}$ ($j$'s attacker $m \in \{m\}$). $F(\{m,j\}) = \{m, j, k\}$ ($k$'s attacker $j \in \{m,j\}$). $F(\{m,j,k\})$: $a$'s attacker is $k \in \{m,j,k\}$, so $a \in F(\{m,j,k\})$. Fixed point: $G = \{m, j, k, a\}$. But $k \in G$ and $(k,a) \in \mathrm{att}$, so $a$ is **refuted** (its attacker $k$ is accepted).
...
$G_t$ is the grounded extension at time $t$: the set of currently accepted artifacts, those whose attackers are all themselves attacked (module 7). The HV-profile at $t$ is the collection of hard-to-vary scores $\mathrm{HV}_B(a)$ for accepted artifacts $a$ at time $t$, measuring how much of each artifact survives genuine variation relative to its battery (module 10). The R-profile at $t$ is the collection of reach sets $R_t(a)$ for accepted artifacts $a$, recording which problems beyond each artifact's origin it currently meets (module 10).
