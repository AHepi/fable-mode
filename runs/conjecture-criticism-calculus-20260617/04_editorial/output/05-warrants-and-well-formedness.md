...
One callback each, then we build: a verdict (pass / fail / overrun) was introduced in module 2; the attack graph $\mathrm{att} \subseteq A \times A$ was built in module 4.
...
**Definition 3.4.** An attack edge $(k, a) \in \mathrm{att}$ must carry a **warrant** of one of two types:
...
**Now the abstract rule.** Let $k$ carry a demonstrative warrant depending on validity node $\nu(\kappa)$. For any artifact $j$ such that $(j, \nu(\kappa)) \in \mathrm{att}$, the closure rule adds $(j, k) \in \mathrm{att}$. Well-formedness requires this hold in every registered demonstrative attack.
...
We now have the ingredients to say when an epistemic state $S = (A, \Pi, \mathrm{addr}, \mathrm{att}, W)$ is *well-formed* — when the whole configuration is internally consistent.

### Definition (Well-Formedness)

A state $S$ is **well-formed** if and only if all four conditions hold:

1. **Warrant registration.** Every attack edge $(k, a) \in \mathrm{att}$ carries a warrant $W(k, a)$ of the kind specified in Definition 3.4 — either demonstrative or argumentative.

2. **Commitment-schema criteria.** For every problem $\pi \in \Pi$, the criteria set $\mathrm{Crit}_\pi$ consists of commitment schemas — specifications that can be instantiated as commitments against candidate artifacts.

3. **Declared addressing.** Every $\mathrm{addr}$ pair $(a, \pi)$ is declared by an addressing artifact: some artifact in $A$ whose content records that $a$ addresses $\pi$. Addressing is not implicit.

4. **Validity-node closure.** For every demonstrative warrant carried by some $k$ depending on $\nu(\kappa)$: for every $j$ with $(j, \nu(\kappa)) \in \mathrm{att}$, we also have $(j, k) \in \mathrm{att}$.

### Bridge: what each condition is doing

Condition 1 is the core of this module: no attack edge exists without a warrant. An edge in $\mathrm{att}$ without a corresponding entry in $W$ would be a bare accusation — the kind of thing Definition 3.4 rules out.
...
**Current attack relation.** $\mathrm{att} = \{(k, a),\, (j, \nu)\}$.

**Well-formedness check.**

- Condition 1: $(k,a)$ has warrant $W(k,a)$ ✓; $(j, \nu)$ has warrant $W(j,\nu)$ ✓.
- Condition 2: assume $\mathrm{Crit}_\pi$ consists of commitment schemas ✓.
- Condition 3: assume $\mathrm{addr}$ pairs are declared ✓.
- Condition 4: $k$ depends on $\nu$, and $(j, \nu) \in \mathrm{att}$. So closure requires $(j, k) \in \mathrm{att}$. But $(j, k) \notin \mathrm{att}$.

**Verdict.** The state is **ill-formed** — condition 4 fails.

**Fix.** Add $(j, k)$ to $\mathrm{att}$ with a warrant (the closure rule does not require a new independent warrant for $(j,k)$; the rule registers the edge as a structural consequence). After adding it:
...
No new edges are added by the closure rule. The closure rule fires only when an artifact attacks $\nu(\kappa)$ — the validity node — not when it attacks $k$ directly. Here $m \to k$ exists by an independent argumentative warrant, not via $\nu(\kappa)$. Closure is a rule about attacks *on the validity node* propagating to the warrant-carrier; attacks on the warrant-carrier itself are separate events, already recorded in $\mathrm{att}$.

</details>

---

**2.** A state has three problems $\pi_1, \pi_2, \pi_3$. The criteria of $\pi_1$ are commitment schemas. The criteria of $\pi_2$ include a natural-language sentence "the conjecture should be elegant." The criteria of $\pi_3$ are empty ($\mathrm{Crit}_{\pi_3} = \emptyset$). Which problems, if any, cause the state to be ill-formed?
...
**4.** State $S$ has $\mathrm{att} = \{(k_1, a), (k_2, a), (j, \nu_1)\}$ where $k_1$ is demonstrative (depending on $\nu_1$) and $k_2$ is argumentative. Is $S$ well-formed? If not, state exactly what edge is missing and why.

<details><summary>Show solution</summary>

$S$ is **ill-formed**. The demonstrative warrant of $k_1$ depends on $\nu_1$, and $(j, \nu_1) \in \mathrm{att}$. By the closure rule (condition 4), $(j, k_1)$ must also be in $\mathrm{att}$. It is not, so condition 4 fails. The argumentative attack $k_2 \to a$ is unaffected — $k_2$ does not depend on $\nu_1$, so closure generates no obligation for $k_2$. The fix: add $(j, k_1)$ to $\mathrm{att}$ with a warrant recording the structural closure.
...
Apply the rule once for each dependency. $\nu(\kappa)$ carries a demonstrative warrant depending on $\nu(\kappa')$, so: $(j, \nu(\kappa')) \in \mathrm{att}$ forces $(j, \nu(\kappa)) \in \mathrm{att}$ (closure for the warrant carried by $\nu(\kappa)$). Now $\nu(\kappa)$ is attacked by $j$, and $k$ carries a demonstrative warrant depending on $\nu(\kappa)$, so: $(j, \nu(\kappa)) \in \mathrm{att}$ forces $(j, k) \in \mathrm{att}$. In total, the closure rule requires adding both $(j, \nu(\kappa))$ and $(j, k)$ to $\mathrm{att}$. The rule propagates along chains of demonstrative dependency: an attack on a validity node propagates to every warrant-carrier that depends on it, transitively.
