...
<!--
OPENING (PROSE). Distinct hook: a courtroom-overturned-on-appeal framing — but careful,
canon forbids imaging G as a judge deciding by opinion/vote. So the hook is about the
*structural* consequence (refute the refuter, the original returns), not about a judge's
verdict. Then intuition, then the (Adj) rule, then Lemma 3.1 statement + bridge + full proof
narrated against M7's iteration, then the worked example, then checks, exercises, recap.
-->
...
\mathrm{status}(a) =
\begin{cases}
\textbf{accepted} & a \in G, \\[2pt]
\textbf{refuted} & \exists\, b \in G : (b, a) \in \mathrm{att}, \\[2pt]
\textbf{suspended} & \text{otherwise.}
\end{cases}
$$
