...
That is the whole construction. The rest of this module is the same idea written precisely enough to compute and to trust. Three things have to become formal: the single "one round" step, the repetition that settles, and the labels we read off the result.

## The worked computation

Before any general definition, let us do the example slowly and in full, because every later definition is just this computation named. Recall the attack graph `att` from module 4: a finite set of artifacts with directed edges, where an edge $(k,a)\in att$ reads "`k` attacks `a`."
...
- `a`: its only attacker is `k`. Is `k` attacked by some member of $\{j\}$? Yes — $(j,k)\in att$ and $j\in\{j\}$. Every attacker of `a` is now knocked down, so $a\in S_2$.
...
The bridge into the notation, piece by piece: $a$ ranges over the artifacts of the current state; "*every attacker of $a$*" means every artifact $b$ with $(b,a)\in att$; and "*attacked by some member of $S$*" means there is a $c\in S$ with $(c,b)\in att$. So $F(S)$ collects exactly the artifacts whose attackers are, one and all, knocked down from within $S$ — one round of standing, with $S$ as the artifacts already admitted.

## Definition (The operator F)

Given an epistemic state with attack relation $att$, the operator $F$ sends a set $S$ of artifacts to
$$
F(S)=\{\,a : \text{every attacker of } a \text{ is attacked by some member of } S\,\}.
$$
Equivalently, $a\in F(S)$ iff for every $b$ with $(b,a)\in att$, there is some $c\in S$ with $(c,b)\in att$.
...
\text{refuted} & \exists\,b\in G:\ (b,a)\in att,\\[2pt]
\text{suspended} & \text{otherwise.}
\end{cases}
$$

In the worked example, $G=\{j,a\}$, so: `j` is **accepted** ($j\in G$); `a` is **accepted** ($a\in G$); and `k` is **refuted** (the member $j\in G$ attacks it, $(j,k)\in att$). The conjecture stands, its critic is refuted, and we read all three labels straight off the set we built.
...
**Setup.** Two artifacts $A=\{a,b\}$ that attack each other: $att=\{(a,b),(b,a)\}$. Each criticizes the other; neither has an outside defender.
...
$S_1=\{p\}$ (from exercise 1). $S_2=F(\{p\})$: `p` in (vacuous); `q`'s attacker `p` — is `p` attacked by a member of $\{p\}$? No (nobody attacks `p`), so `q` out; `r`'s attacker `q` — is `q` attacked by a member of $\{p\}$? Yes, $(p,q)\in att$, so `r` in. $S_2=\{p,r\}$. $S_3=F(\{p,r\})=\{p,r\}$ (same checks). So $G=\{p,r\}$. Statuses: `p` **accepted**, `r` **accepted**, `q` **refuted** (the survivor `p` attacks it). This is the three-node chain again, with the names changed — the middle artifact is refuted, and its target is reinstated.

</details>

**3 (Mechanical).** A single artifact `a` with no attacks at all: $A=\{a\}$, $att=\emptyset$. Compute $G$ and $\mathrm{status}(a)$.
...
**4 (Conceptual, cycle).** Consider a three-cycle: $A=\{a,b,c\}$ with $att=\{(a,b),(b,c),(c,a)\}$ (each attacks the next, around the loop). Compute $G$ and the statuses. Compare with the two-cycle example.
